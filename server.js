...
app.get('/api/aio/feed/:feedKey/latest', async (req, res) => {
  try {
    ...
    const url = `https://io.adafruit.com/api/v2/${AIO_USER}/feeds/${feedKey}/data?limit=1`;
    const r = await fetch(url, { headers: { 'X-AIO-Key': AIO_KEY } });
    const body = await r.json();
    if (r.status === 404) {
      return res.status(404).json({
        error: 'feed_not_found',
        message: `Feed "${feedKey}" was not found for user "${AIO_USER}". Verify the exact feed key (case-sensitive) in Adafruit IO.`,
        aio_response: body,
      });
    }
...
app.get('/api/aio/feed/:feedKey/history', async (req, res) => {
  try {
    ...
    const feedKey = req.params.feedKey;
    const url = `https://io.adafruit.com/api/v2/${AIO_USER}/feeds/${feedKey}/data?limit=${limit}`;
    const r = await fetch(url, { headers: { 'X-AIO-Key': AIO_KEY } });
    const body = await r.json();
    res.status(r.status).json(body);
  } catch (e) {
